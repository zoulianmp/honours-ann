...
def latin_hypercube(dose, margin, n_samples, shuffle=True):
    coord_list = []
    n_per_dose = n_samples/len(dose)

    for d in range(len(dose)):
        min_dim = min(dose[d].shape) - 2*margin
        n_batch = n_per_dose/min_dim
        rem = n_per_dose % min_dim
        ...
        for b in range(n_batch + 1):
            ...
            if b == n_batch:
                ...
                x = rnd.sample(x, min_dim)
                y = rnd.sample(y, min_dim)
                z = rnd.sample(z, min_dim)
