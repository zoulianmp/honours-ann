...
def main(ann3d, model, density_dir, fluence_dir, dose_dir, disp, save):
    ...
    for i in range(len(fluence)):
        pdd = ann3d.ann3d_plot_pdd(dose[i], fluence[i], feed_forward)
        prf = ann3d.ann3d_plot_profile(dose[i], fluence[i], feed_forward)
        if disp:
            pdd.show()
            prf.show()
        if save:
            ...
            pdd.savefig('pdd_' + now + '.png')
            prf.savefig('profile_' + now + '.png')
