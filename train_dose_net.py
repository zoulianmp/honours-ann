...
def main(model='keal', num_epochs=100, batchsize=500):
    ...
    updates = lasagne.updates.nesterov_momentum(
            loss, params, learning_rate=0.001, momentum=0.07)
    ...
    # Now dump the network weights to a file like this:
    now = datetime.datetime.now()
    now = str(now.month) + '-' + str(now.day) + '_' + \
          str(now.hour) + '-' + str(now.minute) + '-' + str(now.second)
    np.savez(model + '_' + now, *lasagne.layers.get_all_param_values(network))
